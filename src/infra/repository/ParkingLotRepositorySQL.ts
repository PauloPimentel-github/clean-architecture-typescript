...
export default class ParkingLotRepositorySQL implements ParkingLotRepository {
  ...
  async getParkingLot(code: string): Promise<ParkingLot> {
    const parkingLotData = await database.oneOrNone("select *, (select count(*) from public.parked_car pc where pc.code = pl.code)::int as occupied_spaces from public.parking_lot pl where pl.code = $1", [code]);
  ...
  async saveParkedCar(code: string, plate: string, date: Date): Promise<void> {
    await database.none("insert into public.parked_car (code, plate, date) values ($1, $2, $3)", [code, plate, date]);
  }
